resource "google_cloud_run_v2_service" "staging_app" {
  ...
  template {
    ...
    volumes {
      ...
      cloud_sql_instance {
        instances = [google_sql_database_instance.staging_db.connection_name]
      }
    }
    ...
    containers {
      ...
      env {
        ...
        value_source {
          secret_key_ref {
            secret  = google_secret_manager_secret.staging_postmark_token.secret_id
            version = "latest"
          }
        }
      }
      ...
      env {
        ...
        value_source {
          secret_key_ref {
            secret  = google_secret_manager_secret.staging_google_client_id.secret_id
            version = "latest"
          }
        }
      }
      ...
      env {
        ...
        value_source {
          secret_key_ref {
            secret  = google_secret_manager_secret.staging_google_client_secret.secret_id
            version = "latest"
          }
        }
      }
      ...
      env {
        ...
        value_source {
          secret_key_ref {
            secret  = google_secret_manager_secret.staging_microsoft_client_id.secret_id
            version = "latest"
          }
        }
      }
      ...
      env {
        ...
        value_source {
          secret_key_ref {
            secret  = google_secret_manager_secret.staging_microsoft_client_secret.secret_id
            version = "latest"
          }
        }
      }
      ...
      env {
        name  = "DB_HOST"
        value = "/cloudsql/${google_sql_database_instance.staging_db.connection_name}"
      }

      env {
        name  = "DB_NAME"
        value = google_sql_database.staging_database.name
      }

      env {
        name  = "DB_USER"
        value = google_sql_user.staging_user.name
      }
...
# ------------------------------
resource "google_cloud_run_v2_service" "staging_worker_app" {
  project  = var.project_id
